...
| `mobile-menu` | Mobile menu — **floating glass drawer** (`component-mobile-menu.css`): slides in from the left, inset from every edge with rounded corners, over a dimmed scrim; nav + `<details>` accordions + featured tiles + social row + colophon | reads settings/menus | **Stable** — is a `<theme-drawer>` opened by the header hamburger. Adopts the scrim‑owns‑opacity / panel‑slides‑via‑transform compositing pattern (shared with the cart drawer + search) and the house type scale. (The bottom `dock` was retired — navigation moved into the header.) |
| `search-overlay` | Search — **glass dropdown from the header** (`component-search.css`): the page stays visible and dimmed behind it; scrim + transform‑slid panel | reads settings | **Stable** — pairs with `predictive-search.js`; same compositing pattern as the drawers |
...
| `cart-line` | One cart line row (media, title, colour-coded attribute pills, variant, qty stepper, remove, price) | `item` (req), `index` (badge — page only), `compact` (bool — drawer variant) | **Stable** — shared by the cart drawer (`component-cart-drawer.css`) + cart page; shared row rules in `component-cart.css` |
